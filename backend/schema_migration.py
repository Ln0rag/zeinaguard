...
SCHEMA_STATEMENTS = [
    ...
    "ALTER TABLE threats ADD COLUMN IF NOT EXISTS is_auto_mitigated BOOLEAN DEFAULT FALSE",
    "ALTER TABLE threats ADD COLUMN IF NOT EXISTS auto_mitigated_at TIMESTAMP",
    "ALTER TABLE threats ADD COLUMN IF NOT EXISTS mitigated_by_sensor_id INTEGER",
    """
    CREATE TABLE IF NOT EXISTS mitigation_audits (
        id SERIAL PRIMARY KEY,
        threat_id INTEGER REFERENCES threats(id) ON DELETE CASCADE NOT NULL,
        bssid VARCHAR(17) NOT NULL,
    ...
    """,
    "CREATE INDEX IF NOT EXISTS idx_mitigation_audits_threat ON mitigation_audits(threat_id)",
    "CREATE INDEX IF NOT EXISTS idx_mitigation_audits_sensor_time ON mitigation_audits(sensor_id, timestamp)",


    "CREATE INDEX IF NOT EXISTS idx_threats_dedup_lookup ON threats(source_mac, threat_type, is_resolved, created_at DESC)",


    "CREATE INDEX IF NOT EXISTS idx_wifi_networks_bssid_sensor ON wifi_networks(bssid, sensor_id)",


    "CREATE INDEX IF NOT EXISTS idx_wifi_networks_active_partial ON wifi_networks(last_seen) WHERE is_active = TRUE",
]
