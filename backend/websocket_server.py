...
def run_cleanup_cycle() -> tuple[int, int, int]:
    ...
    try:
        # بنخلي البايثون يمسح الشبكات القديمة بس (State Table)
        ...
        # وقفنا مسح الـ Threats والـ NetworkScanEvents عشان الـ TimescaleDB Retention Policies هي اللي هتديرهم للمدى الطويل
        db.session.commit()
    except Exception:
        db.session.rollback()
        raise
...
def _emit_snapshot(socketio: SocketIO, room: str | None = None) -> None:
    """
    تقوم هذه الدالة بإرسال حالة الشبكة والحساسات الحالية.
    تم تعديلها لتقوم بالإرسال العام (Broadcast) لضمان مزامنة القائمة البيضاء
    مع جميع الحساسات حتى لو كانت في غرف معزولة.
    """
    ...
    # تم إزالة 'room=room' من الاستدعاءات أدناه لتفعيل الـ Broadcast للجميع
    _emit_socket_event(socketio, NETWORK_SNAPSHOT_EVENT, network_snapshot)
    _emit_socket_event(socketio, NETWORKS_SNAPSHOT_EVENT, networks_snapshot)
    _emit_socket_event(socketio, SENSOR_SNAPSHOT_EVENT, sensor_snapshot)
...
def init_socketio(app):
    ...
    def handle_network_scan(payload):
        ...
        # الدالة بقت بره الـ for loop عشان تبعت الدفعة كلها مرة واحدة
        _emit_snapshot(socketio)
    ...
    def handle_new_threat(payload):
        ...
        try:
            ...
            new_threat = Threat(
                ...
                action_status=payload.get("action_status", "IDLE")
            )
    ...
    def handle_attack_ack(payload):
        payload = sanitize_json_payload(payload or {})
        # بنذيع الـ payload زي ما هو بكل تفاصيله (SSID, Channel, Signal) للداشبورد
