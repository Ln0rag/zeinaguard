...
from sqlalchemy import func, literal_column, select, text, case, and_
...
from realtime_state import (
    ...
    force_trust_bssid,
    is_bssid_trusted,
    get_best_sensor_for_bssid,
    periodic_evil_twin_scan,
)
...
def _sanity_check_server_env():
    if not JWT_SECRET_KEY:
        LOGGER.critical("[FATAL] JWT_SECRET_KEY is not set in environment variables. Refusing to start insecurely.")
        sys.exit(1)

_sanity_check_server_env()
...
def verify_token(token: str) -> dict:
    """Verify the JWT token and return the decoded payload. Raises PyJWT errors on failure."""
    # Enforces the HS256 algorithm and the exactly matched JWT_SECRET_KEY
    return jwt.decode(token, JWT_SECRET_KEY, algorithms=["HS256"])

active_networks_cache: dict[str, float] = {}
_active_networks_cache_lock = threading.Lock()

UPTIME_PART_PATTERN = re.compile(r"(\d+)\s*([dhms])", re.IGNORECASE)
connected_clients: dict[str, dict[str, Any]] = {}
_connected_clients_lock = threading.Lock()

SENSOR_REGISTRATION_KEY: str = os.getenv("SENSOR_REGISTRATION_KEY", "")
...
THREAT_DEDUPE_WINDOW_SECONDS = int(os.getenv("THREAT_DEDUPE_WINDOW_SECONDS", "3600"))
...
EVIL_TWIN_SUSPECTED_EVENT = "evil_twin_suspected"
TRUSTED_AP_ANOMALY_EVENT = "trusted_ap_anomaly"
...
_evil_twin_periodic_cache: dict[str, float] = {}
_EVIL_TWIN_PERIODIC_INTERVAL_SECONDS = 30
_EVIL_TWIN_PERIODIC_COOLDOWN_SECONDS = 300  # 5 minutes between re-emits per BSSID
_TRUSTED_SECURITY_CHECK_INTERVAL_SECONDS = 30  # seconds between trusted-network security sweeps
...
class ScanPersistenceManager:
    ...
    def _build_queued_event(self, network_data: dict[str, Any]) -> QueuedNetworkEvent:
        ...
        network_data.pop("first_seen", None)
        network_data.pop("created_at", None)
        network_data.pop("last_seen", None)
        network_data.pop("updated_at", None)
        network_data.pop("timestamp", None)
        ...
        wps_status = _get_wps_status_helper(wps_val)
        calculated_severity = calculate_network_severity(ssid, encryption if encryption != "UNKNOWN" else auth_type, wps_status)
        
        severity = _normalize_severity(network_data.get("severity"), classification)
        if severity == "medium" and calculated_severity != "medium":
    ...
    def _merge_update(self, pending: BufferedNetworkUpdate, incoming: BufferedNetworkUpdate) -> None:
        def is_invalid(val):
            return not val or str(val).strip().lower() in ['', 'hidden', 'none', 'unknown', 'n/a']

        if not is_invalid(pending.ssid) and is_invalid(incoming.ssid):
            pass
        else:
            pending.ssid = incoming.ssid
            
        if not is_invalid(pending.encryption) and is_invalid(incoming.encryption):
            pass
        else:
    ...
    def flush(self) -> None:
        ...
        try:
            ...
            with self._stats_lock:
                self._stats["dropped"] += dropped_count
            LOGGER.error(
                "[DB] Batch flush failed — %d networks re-queued for next cycle: %s",
                dropped_count, exc, exc_info=True,
            )
        finally:
            self._log_periodic_summary()
            db.session.remove()
    ...
    def _flush_batch(self, batch: dict[tuple[int, str], BufferedNetworkUpdate]) -> FlushResult:
        ...
        wifi_rows = [
            {
                ...
                "first_seen": datetime.utcnow(), "last_seen": datetime.utcnow(), "is_active": True, "raw_beacon": update.raw_beacon,
                "raw_data": update.raw_data, "created_at": datetime.utcnow(), "updated_at": datetime.utcnow(),
        ...
        upsert_stmt = insert_stmt.on_conflict_do_update(
            ...
            set_={
                "ssid": case(
                    (
                        and_(
                            func.lower(wifi_table.c.ssid).notin_(['', 'hidden', 'none', 'unknown', 'n/a']),
                            func.lower(insert_stmt.excluded.ssid).in_(['', 'hidden', 'none', 'unknown', 'n/a'])
                        ),
                        wifi_table.c.ssid
                    ),
                    else_=insert_stmt.excluded.ssid
                ),
                "encryption": case(
                    (
                        and_(
                            func.lower(wifi_table.c.encryption).notin_(['', 'hidden', 'none', 'unknown', 'n/a']),
                            func.lower(insert_stmt.excluded.encryption).in_(['', 'hidden', 'none', 'unknown', 'n/a'])
                        ),
                        wifi_table.c.encryption
                    ),
                    else_=insert_stmt.excluded.encryption
                ),
                "channel": insert_stmt.excluded.channel, "frequency": insert_stmt.excluded.frequency,
                ...
                "last_seen": datetime.utcnow(), "is_active": True, "updated_at": datetime.utcnow(),
        ...
        scan_rows = [
            {
                ...
                "reasons": update.reasons, "scanned_at": datetime.utcnow(),
            } for update in updates
        ]
        ...
        _threat_worthy_classifications = {"ROGUE", "SUSPICIOUS"}
        if inserted_updates:
            threat_rows = []
            now = datetime.utcnow()
            for update in inserted_updates:
                if (update.classification or "").upper() not in _threat_worthy_classifications:
                    continue
                threat_rows.append({
                    "threat_type": _normalize_threat_type(update.classification),
                    "severity": _normalize_severity(update.severity, update.classification),
                    "source_mac": update.bssid,
                    "ssid": update.ssid,
                    "description": "Detected via Proactive Network Scan",
                    "detected_by": update.sensor_id,
                    "created_at": now,
                    "is_resolved": False,
                    "is_auto_mitigated": False
                })
            if threat_rows:
                db.session.execute(Threat.__table__.insert(), threat_rows)
    ...
    def _flush_batch_generic(self, batch: dict[tuple[int, str], BufferedNetworkUpdate]) -> FlushResult:
        ...
        for key, update in batch.items():
            ...
            if existing is None:
                existing = WiFiNetwork(
                    ...
                    ap_uptime=update.uptime_seconds,
                    raw_beacon=update.raw_beacon, raw_data=update.raw_data, seen_count=update.seen_increment, first_seen=datetime.utcnow(),
                ...
                existing.ap_uptime = update.uptime_seconds
        ...
        if inserted_updates:
            ...
            db.session.add_all(threat_objs)
...
def _normalize_classification(value: Any) -> str:
    normalized = sanitize_input(str(value or "UNKNOWN"), max_length=50).upper()
    if normalized not in {"ROGUE", "SUSPICIOUS", "LEGIT", "UNKNOWN"}:
        return "UNKNOWN"
    return normalized

def _get_wps_status_helper(wps_info):
...
def calculate_network_severity(ssid: str, security: str, wps_status: str) -> str:
    sec = str(security or "").upper()
    s = str(ssid or "").strip()
    
    if sec in ["OPEN", "NONE", "WEP"] or wps_status == "UNLOCKED":
        return "high"
        
    if not s or s.lower() in ["hidden", "<length: 0>"]:
        return "medium"
...
def _find_recent_duplicate_threat(threat_type: str, source_mac: str | None) -> Threat | None:
    if not source_mac:
        return None
    return Threat.query.filter(Threat.source_mac == source_mac, Threat.threat_type == threat_type, Threat.is_resolved == False).order_by(Threat.created_at.desc()).first()

def _persist_evil_twin_threat(anomaly_data: dict[str, Any], sensor_id: int) -> None:

    """Persist an evil twin detection as a Threat record with deduplication.
    Creates a new 'critical' Threat row for the rogue BSSID, or touches the
    existing unresolved record (updated_at) so it stays fresh in the UI.
    """
    
    bssid = _normalize_bssid(anomaly_data.get("bssid") or "")
    if not bssid:
        return
    ssid = str(anomaly_data.get("ssid") or "Hidden")
    trusted_originals: list[str] = anomaly_data.get("trusted_originals") or []
    threat_type = "evil_twin"

    existing = _find_recent_duplicate_threat(threat_type, bssid)
    if existing:
        existing.updated_at = datetime.utcnow()
        db.session.commit()
        return
    ...
    description = (
        f"Evil Twin: SSID '{ssid}' spoofed by {bssid}. "
        f"Impersonating trusted AP(s): {', '.join(trusted_originals)}"
    )
    threat = Threat(
...
def _periodic_evil_twin_check_loop(app, socketio: SocketIO) -> None:

    """Background task: re-validates all active networks for evil twins every 30 s.
    Detects newly appearing evil twins and survives backend/sensor restarts.
    Uses a per-BSSID cooldown to avoid re-emitting the same alert every cycle.
    """

    while True:
        socketio.sleep(_EVIL_TWIN_PERIODIC_INTERVAL_SECONDS)
        try:
            anomalies = periodic_evil_twin_scan()
            if not anomalies:
                continue
            now = time.monotonic()
            for anomaly in anomalies:
                bssid = (anomaly.get("data") or {}).get("bssid", "")
                last_emitted = _evil_twin_periodic_cache.get(bssid)
                if last_emitted and (now - last_emitted) < _EVIL_TWIN_PERIODIC_COOLDOWN_SECONDS:
                    continue
                _evil_twin_periodic_cache[bssid] = now
                # Emit live alert to dashboard
                _emit_socket_event(socketio, anomaly["type"], anomaly["data"], room=DASHBOARD_ROOM)
                # Persist to DB within an app context
                with app.app_context():
                    try:
                        _persist_evil_twin_threat(anomaly["data"], 0)
                    except Exception as _exc:
                        LOGGER.warning("[EvilTwin Periodic] DB persist failed for %s: %s", bssid, _exc)
                        db.session.rollback()
                    finally:
                        db.session.remove()
        except Exception as _exc:
            LOGGER.warning("[EvilTwin Periodic] Scan cycle failed: %s", _exc)


def _persist_trusted_ap_anomaly(anomaly_data: dict[str, Any], sensor_id: int) -> None:
    """Persist a trusted-AP anomaly (channel drift or security degradation) as a Threat record."""
    bssid = _normalize_bssid(anomaly_data.get("bssid") or "")
    if not bssid:
        return
    ssid = str(anomaly_data.get("ssid") or "Hidden")
    anomaly_kind = str(anomaly_data.get("anomaly_kind") or "unknown")
    is_trusted = bool(anomaly_data.get("is_trusted", False))
    baseline = anomaly_data.get("baseline_value", "?")
    current_val = anomaly_data.get("current_value", "?")

    if anomaly_kind == "channel_change":
        threat_type = "channel_drift"
        severity = "medium"
        description = (
            f"Trusted AP '{ssid}' ({bssid}) switched channel from "
            f"{baseline} to {current_val} — possible rogue or ACS fault."
        )
    elif anomaly_kind == "security_degradation":
        threat_type = "security_degradation"
        severity = "critical" if is_trusted else "high"
        description = (
            f"Trusted AP '{ssid}' ({bssid}) security downgraded from "
            f"{baseline} to {current_val} — possible evil twin or misconfiguration."
        )
    else:
        threat_type = "trusted_ap_anomaly"
        severity = "medium"
        description = f"AP '{ssid}' ({bssid}) anomaly: {anomaly_kind}"
...
def _persist_trusted_security_warning(bssid: str, ssid: str, threat_type: str, severity: str, description: str, sensor_id: int) -> None:

    """Persist a trusted network security warning as a Threat record with deduplication."""
    
    bssid_norm = _normalize_bssid(bssid)
    if not bssid_norm:
        return
    existing = _find_recent_duplicate_threat(threat_type, bssid_norm)
    if existing:
        existing.updated_at = datetime.utcnow()
        db.session.commit()
        return
...
def _check_trusted_network_security_issues(network: dict[str, Any]) -> list[dict[str, Any]]:

    """Return security issues detected in a trusted network's live data.
    Checks for open auth, WEP, weak encryption (TKIP/WPA1), and WPS enabled.
    Returns structured issue dicts consumed by _periodic_trusted_network_check_loop.
    """

    issues: list[dict[str, Any]] = []
    ssid = str(network.get("ssid") or "Hidden")
    auth = str(network.get("auth") or "").strip().upper()
    encryption = str(network.get("encryption") or "").strip().upper()
    wps = str(network.get("wps") or "").strip().upper()

    is_open = auth in ("OPEN", "NONE", "") or encryption in ("OPEN", "NONE", "")
    has_wep = "WEP" in encryption or "WEP" in auth
    has_tkip = "TKIP" in encryption
    has_wpa1_only = (
        "WPA" in auth
        and "WPA2" not in auth and "WPA3" not in auth
        and "WPA2" not in encryption and "WPA3" not in encryption
    )
    wps_enabled = wps not in ("DISABLED", "UNKNOWN", "NONE", "", "LOCKED")

    if is_open:
        issues.append({
            "issue_key": "open",
            "severity": "critical",
            "threat_type": "trusted_open_network",
            "current_encryption": encryption or "OPEN",
            "message": f'"{ssid}" is OPEN >> anyone can connect.',
        })
    elif has_wep:
        issues.append({
            "issue_key": "wep",
            ...
            "message": f'"{ssid}" uses WEP >> Upgrade to WPA2/WPA3 immediately.',
        })
    elif has_tkip or has_wpa1_only:
        issues.append({
            "issue_key": "weak_enc",
            "severity": "high",
            "threat_type": "weak_encryption",
            "current_encryption": encryption or auth,
            "message": f'"{ssid}" uses weak encryption ({encryption or auth}) >> Upgrade to WPA2/WPA3.',
        })
    ...
    if wps_enabled:
        issues.append({
            "issue_key": "wps",
            "severity": "medium",
            "threat_type": "wps_vulnerability",
            "current_encryption": encryption,
            "message": f'"{ssid}" has WPS enabled and vulnerable to PIN brute-force attacks.',
        })

    return issues


def _periodic_trusted_network_check_loop(app, socketio: SocketIO) -> None:

    """Background task: checks all trusted networks for insecure configs every 30 s.
    Only evaluates networks listed in trusted.json that are currently active/visible.
    Emits 'trusted_weak_encryption' live alerts and persists each finding to the
    Threat table with deduplication (updated_at is touched on re-occurrence).
    """
    
    while True:
        socketio.sleep(_TRUSTED_SECURITY_CHECK_INTERVAL_SECONDS)
        try:
            ...
            for t_bssid in trusted_dict:
                network = get_realtime_network(t_bssid)
                if not network:
                    continue

                issues = _check_trusted_network_security_issues(network)
                if not issues:
                    continue

                bssid_norm = _normalize_bssid(t_bssid)
                ssid = str(network.get("ssid") or trusted_dict.get(t_bssid) or "Hidden")
                sensor_id = _safe_int(network.get("sensor_id"), default=0) or 0

                for issue in issues:
                    event_data = {
                        "bssid": bssid_norm,
                        "ssid": ssid,
                        "current_encryption": issue.get("current_encryption", "UNKNOWN"),
                        "expected_encryption": "WPA2/WPA3",
                        "severity": "critical" if issue["issue_key"] in ("open", "wep") else "warning",
                        "message": issue["message"],
                        "threat_type": issue["threat_type"],
                        "timestamp": _utc_iso(),
                    }
                    _emit_socket_event(socketio, "trusted_weak_encryption", event_data, room=DASHBOARD_ROOM)

                    with app.app_context():
                        try:
                            _persist_trusted_security_warning(
                                bssid=bssid_norm,
                                ssid=ssid,
                                threat_type=issue["threat_type"],
                            ...
                            LOGGER.warning("[TrustedSecurity] DB persist failed %s: %s", bssid_norm, _exc)
                            db.session.rollback()
                        finally:
                            db.session.remove()

        except Exception as _exc:
            LOGGER.warning("[TrustedSecurity] Check cycle failed: %s", _exc)
...
def _resolve_sensor(
    sensor_identifier: Any,
    hostname: Any = None,
    registration_key: str | None = None,
) -> Sensor:
    
    """Resolve or create a Sensor record.
      - If the sensor already exists (found by ID, hostname, or name) it is
        returned unconditionally — re-registration of a known sensor never
        requires the key.
      - Auto-creation of a *new* Sensor is gated on SENSOR_REGISTRATION_KEY.
        When the env var is set, the caller must supply the correct key;
        mismatches raise ValueError so the register handler can reject cleanly
        without silently writing a phantom row to the DB.
      - When SENSOR_REGISTRATION_KEY is empty (development / open mode) the
        behaviour is unchanged for backwards compatibility.
    """
    ...
    if sensor is None:
        # Guard: only auto-create when the SENSOR_REGISTRATION_KEY matches.
        if SENSOR_REGISTRATION_KEY and registration_key != SENSOR_REGISTRATION_KEY:
            raise ValueError(
                "sensor_register rejected: invalid or missing SENSOR_REGISTRATION_KEY"
            )
...
def get_sensor_socket_id(sensor_id: int) -> str | None:
    ...
    for sid, client in clients_snapshot:
...
def dispatch_attack_command(socketio: SocketIO, payload: dict[str, Any]) -> tuple[dict[str, Any], int]:
    payload = sanitize_json_payload(payload or {})
    requested_sensor_id = _safe_int(payload.get("sensor_id"), default=0)
    ...
    if not target_bssid:
        return {"status": "error", "message": "attack_command missing target_bssid", "timestamp": _utc_iso()}, 400
    if channel is None:
        return {"status": "error", "target_bssid": target_bssid, "message": "attack_command missing channel", "timestamp": _utc_iso()}, 400

    best_sensor_id = get_best_sensor_for_bssid(target_bssid)
    sensor_id = (
        best_sensor_id
        if best_sensor_id and is_sensor_connected(best_sensor_id)
        else requested_sensor_id
    ) or requested_sensor_id
    routed_via_consensus = bool(best_sensor_id and best_sensor_id == sensor_id)
    ...
    _emit_socket_event(socketio, ATTACK_COMMAND_EVENT, {**command_payload, "timestamp": _utc_iso(), "status": "dispatched", "routed_via_consensus": routed_via_consensus}, room=DASHBOARD_ROOM)
    _emit_socket_event(socketio, EXECUTE_ATTACK_EVENT, command_payload, room=sensor_sid)
    return {"status": "ok", "sensor_id": sensor_id, "target_bssid": target_bssid, "channel": channel, "routed_via_consensus": routed_via_consensus, "timestamp": _utc_iso()}, 200
...
def _format_networks_snapshot_item(network: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "first_seen": network.get("first_seen") or network.get("created_at"),
    }
...
def _apply_row_cap(model, time_column, max_rows: int) -> int:
    ...
    return (
        ...
        .filter(model.id.in_(select(to_delete_cte.c.id)))
...
def run_cleanup_cycle() -> tuple[int, int, int]:
    ...
    try:
        deleted_networks = db.session.query(WiFiNetwork).filter(
            WiFiNetwork.last_seen < func.now() - text(f"INTERVAL '{NETWORK_RETENTION_HOURS} hours'")
        ).delete(synchronize_session=False)
...
def _cleanup_loop(app, socketio) -> None:
    while True:
...
def start_cleanup_thread(app, socketio) -> None:
    global _cleanup_thread_started
    with _cleanup_thread_lock:
        if _cleanup_thread_started:
            return
        socketio.start_background_task(_cleanup_loop, app, socketio)
...
def _mark_stale_network_rows_inactive() -> int:
    try:
        ...
        stale_records = db.session.query(WiFiNetwork.id).filter(
        ...
        updated_rows = db.session.query(WiFiNetwork).filter(
            WiFiNetwork.id.in_(stale_ids)
        ).update(
            {WiFiNetwork.is_active: False, WiFiNetwork.updated_at: datetime.utcnow()}, 
...
_SNAPSHOT_HISTORY_HOURS = int(os.getenv("SNAPSHOT_HISTORY_HOURS", "24"))


def _emit_snapshot(socketio: SocketIO, room: str | None = None) -> None:
    """
    Emit current network + sensor state.
    When `room` is a specific SID (e.g. on initial connect), send only to that client.
    When `room` is None, broadcast to DASHBOARD_ROOM (dashboard clients only —
    sensors leave DASHBOARD_ROOM after registering so they are excluded).

    On initial connect (room is a specific SID) we enrich the live snapshot with
    DB-backed historical data so a freshly-opened tab is never blank:
      • first_seen timestamps are backfilled from DB for every live network.
      • DB networks seen in the last SNAPSHOT_HISTORY_HOURS that are NOT already
        in the realtime state are appended so historical entries are visible
        immediately, before any new scan arrives.
    """
    target_room = room if room is not None else DASHBOARD_ROOM

    live_networks = get_realtime_network_snapshot()

    if room is not None:
        live_bssids: set[str] = {n["bssid"] for n in live_networks if "bssid" in n}
        try:
            history_cutoff = datetime.utcnow() - timedelta(hours=_SNAPSHOT_HISTORY_HOURS)
            db_nets = WiFiNetwork.query.filter(
                WiFiNetwork.last_seen >= history_cutoff,
            ).order_by(WiFiNetwork.last_seen.desc()).limit(2000).all()

            created_map = {n.bssid: n.first_seen for n in db_nets if n.first_seen}
            for n in live_networks:
                if n.get("bssid") in created_map:
                    n["first_seen"] = created_map[n["bssid"]].isoformat() + "Z"

            for db_net in db_nets:
                if db_net.bssid not in live_bssids:
                    raw_data = db_net.raw_data or {}
                    live_networks.append({
                        "bssid": db_net.bssid,
                        "ssid": db_net.ssid or "Hidden",
                        "sensor_id": db_net.sensor_id,
                        "classification": (db_net.classification or "LEGIT").upper(),
                        "signal": db_net.signal_strength or 0,
                        "channel": db_net.channel or 0,
                        "frequency": db_net.frequency or 0,
                        "encryption": db_net.encryption or "Unknown",
                        "auth": db_net.auth_type,
                        "wps": db_net.wps_info,
                        "manufacturer": db_net.manufacturer,
                        "clients_count": max(db_net.clients_count or 0, 0),
                        "uptime": db_net.uptime_seconds or 0,
                        "score": db_net.risk_score or 0,
                        "reasons": [],
                        "is_trusted": False,
                        "was_hidden": bool(raw_data.get("was_hidden", False)),
                        "last_seen": db_net.last_seen.isoformat() + "Z" if db_net.last_seen else None,
                        "timestamp": db_net.last_seen.isoformat() + "Z" if db_net.last_seen else None,
                        "first_seen": db_net.first_seen.isoformat() + "Z" if db_net.first_seen else None,
                        "is_active": False,
                    })
                    live_bssids.add(db_net.bssid)

        except Exception as e:
            LOGGER.error("Failed to enrich snapshot with DB history: %s", e)
            db.session.rollback()
    ...
    _emit_socket_event(socketio, NETWORK_SNAPSHOT_EVENT, network_snapshot, room=target_room)
    _emit_socket_event(socketio, NETWORKS_SNAPSHOT_EVENT, networks_snapshot, room=target_room)
    _emit_socket_event(socketio, SENSOR_SNAPSHOT_EVENT, sensor_snapshot, room=target_room)
...
def mark_network_trusted(socketio: SocketIO | None, bssid: str) -> dict[str, Any] | None:
    ...
    if network_data:
        ...
        _, network_data, _trust_anomalies = upsert_realtime_network(network_data)
        if socketio and _trust_anomalies:
            for _anomaly in _trust_anomalies:
                _emit_socket_event(
                    socketio,
                    _anomaly["type"],
                    _anomaly["data"],
...
def _in_memory_ttl_sweeper(app, socketio: SocketIO):
    while True:
        socketio.sleep(5)
        now = time.time()
        expired_bssids = []

        with _active_networks_cache_lock:
            snapshot_items = list(active_networks_cache.items())
        for bssid, last_seen in snapshot_items:
            if now - last_seen > 15:
                expired_bssids.append(bssid)
        if expired_bssids:
            with _active_networks_cache_lock:
                for bssid in expired_bssids:
                    active_networks_cache.pop(bssid, None)
                
        if expired_bssids:
            with app.app_context():
                try:
                    db.session.query(WiFiNetwork).filter(
                        WiFiNetwork.bssid.in_(expired_bssids)
                    ).update(
                        {WiFiNetwork.is_active: False, WiFiNetwork.updated_at: datetime.utcnow()}, 
                        synchronize_session=False
                    )
                    ...
                    db.session.rollback()
                    LOGGER.error(f"[TTL Cache] DB update failed: {exc}")
                finally:
                    db.session.remove()
                    
            for bssid in expired_bssids:
                socketio.emit("status_update", {"type": "status_update", "bssid": bssid, "status": "OFF"}, room=DASHBOARD_ROOM)
...
def start_realtime_state_thread(app, socketio: SocketIO) -> None:
    ...
    with _realtime_state_thread_lock:
        if _realtime_state_thread_started:
            return
        socketio.start_background_task(_realtime_state_loop, app, socketio)
        socketio.start_background_task(_in_memory_ttl_sweeper, app, socketio)
        # Task: Periodic evil twin re-validation every 30 s
        socketio.start_background_task(_periodic_evil_twin_check_loop, app, socketio)
        # Task: Periodic trusted network security warnings every 30 s
        socketio.start_background_task(_periodic_trusted_network_check_loop, app, socketio)
...
def init_socketio(app):
    ...
    start_cleanup_thread(app, socketio)
    start_realtime_state_thread(app, socketio)
    persistence_manager = _get_persistence_manager(app)

    @socketio.on("connect")
    def handle_connect(auth=None):
        LOGGER.info(f"[SOCKET.IO] New connection attempt. sid={request.sid}")
        
        from flask_socketio import ConnectionRefusedError
        if auth is None:
            LOGGER.error(f"[SECURITY] Handshake rejected sid={request.sid}: 'auth' object is entirely missing.")
            raise ConnectionRefusedError("Authentication payload is missing.")
            
        if not isinstance(auth, dict):
            LOGGER.error(f"[SECURITY] Handshake rejected sid={request.sid}: 'auth' is not a dictionary. Received type: {type(auth).__name__}.")
            raise ConnectionRefusedError("Authentication payload must be a dictionary.")

        if 'token' not in auth:
            LOGGER.error(f"[SECURITY] Handshake rejected sid={request.sid}: Missing 'token' key in auth dictionary.")
            raise ConnectionRefusedError("The 'token' key is missing from the authentication payload.")
            
        token = auth['token']
        if not token:
            LOGGER.error(f"[SECURITY] Handshake rejected sid={request.sid}: Token value is empty.")
            raise ConnectionRefusedError("Token value cannot be empty.")
            
        try:
            decoded = verify_token(token)
            LOGGER.info(f"[SECURITY] Handshake SUCCESS sid={request.sid} for client_type: {decoded.get('client_type', 'unknown')}")
        except jwt.ExpiredSignatureError:
            LOGGER.error(f"[SECURITY] Handshake rejected sid={request.sid}: JWT Token has EXPIRED.")
            raise ConnectionRefusedError("JWT Token has expired.")
        except jwt.InvalidSignatureError:
            LOGGER.error(f"[SECURITY] Handshake rejected sid={request.sid}: JWT Invalid Signature. Check JWT_SECRET_KEY mismatch.")
            raise ConnectionRefusedError("JWT Invalid Signature.")
        except jwt.InvalidTokenError as e:
            LOGGER.error(f"[SECURITY] Handshake rejected sid={request.sid}: JWT Invalid Token Error: {str(e)}")
            raise ConnectionRefusedError(f"JWT Invalid Token Error: {str(e)}")
            
        client_id = request.sid
        join_room(DASHBOARD_ROOM)
        with _connected_clients_lock:
            connected_clients[client_id] = {"connected_at": _utc_iso(), "client_type": decoded.get("client_type", "dashboard")}
    ...
    def handle_sensor_register(data):
        ...
        with _connected_clients_lock:
            caller_type = (connected_clients.get(request.sid) or {}).get("client_type", "dashboard")
        if caller_type == "dashboard":
            LOGGER.warning(
                "[SECURITY] sensor_register rejected: dashboard client sid=%s attempted sensor role assumption",
                request.sid,
            )
            _emit_context_event("registration_error", {"status": "error", "message": "dashboard_cannot_register_as_sensor"})
            return
        try:
            reg_key = sanitize_input(str(data.get("registration_key") or ""), max_length=255) or None
            sensor = _resolve_sensor(
                sensor_identifier=reg_key or data.get("sensor_id"),
            ...
            _handle_sensor_presence(socketio, {"sensor_id": sensor.id, "registration_key": reg_key, "hostname": sensor.hostname, "status": "online", "message": "Sensor registered", "interface": data.get("interface")}, sid=request.sid, status_event_name="sensor_register")
        except Exception as exc:
            db.session.rollback()
    ...
    def handle_network_scan(payload):
        try:
            ...
            for network_data in network_events:
                bssid_raw = network_data.get("bssid") or network_data.get("source_mac")
                if bssid_raw:
                    with _active_networks_cache_lock:
                        active_networks_cache[_normalize_bssid(bssid_raw)] = time.time()
                try:
                    ...
                    action, live_network, _scan_anomalies = upsert_realtime_network(network_data)
                    for _anomaly in _scan_anomalies:
                        _emit_socket_event(
                            socketio,
                            _anomaly["type"],
                            _anomaly["data"],
                        ...
                        if _anomaly["type"] == "evil_twin_suspected":
                            try:
                                _persist_evil_twin_threat(_anomaly["data"], sensor_id)
                            except Exception as _et_exc:
                                LOGGER.warning("[EvilTwin] DB persist failed: %s", _et_exc)
                                db.session.rollback()
                        elif _anomaly["type"] == "trusted_ap_anomaly":
                            try:
                                _persist_trusted_ap_anomaly(_anomaly["data"], sensor_id)
                            except Exception as _ta_exc:
                                LOGGER.warning("[TrustedApAnomaly] DB persist failed: %s", _ta_exc)
                                db.session.rollback()
            ...
            with _connected_clients_lock:
                if request.sid in connected_clients:
                    connected_clients[request.sid]["last_seen"] = datetime.utcnow().isoformat() + "Z"

            _emit_context_event("network_scan_ack", {"status": "ok" if queued else "partial", "queued": queued, "dropped": dropped, "timestamp": datetime.utcnow().isoformat() + "Z"})
        finally:
            db.session.remove()
    ...
    def handle_new_threat(payload):
        ...
        try:
            ...
            if duplicate_threat is not None:
                # SSID Amnesia Protection for Threat Table
                if duplicate_threat.ssid and duplicate_threat.ssid.strip().lower() not in ['', 'hidden', 'none', 'unknown', 'n/a']:
                    if not ssid or ssid.strip().lower() in ['', 'hidden', 'none', 'unknown', 'n/a']:
                        ssid = duplicate_threat.ssid
                
                duplicate_threat.ssid = ssid
                duplicate_threat.updated_at = datetime.utcnow()
                
                # Update severity if the incoming payload has a valid one
                payload_severity = payload.get("severity")
                if payload_severity:
                    duplicate_threat.severity = _normalize_severity(payload_severity, classification)
                
                db.session.commit()
                
                # Update frontend for existing threat
                _emit_socket_event(socketio, "threat_event", {
                    "id": duplicate_threat.id, 
                    "type": THREAT_DETECTED_EVENT, 
                    "timestamp": _utc_iso(), 
                    "severity": duplicate_threat.severity, 
                    "data": {
                        "id": duplicate_threat.id, 
                        "threat_type": duplicate_threat.threat_type, 
                        "severity": duplicate_threat.severity, 
                        "source_mac": duplicate_threat.source_mac, 
                        "ssid": duplicate_threat.ssid, 
                        "detected_by": duplicate_threat.detected_by, 
                        "description": duplicate_threat.description, 
                        "signal": _safe_int(payload.get("signal"), default=0) or None,
                        "channel": _safe_int(payload.get("channel"), default=0) or None,
                        "is_resolved": duplicate_threat.is_resolved, 
                        "created_at": duplicate_threat.created_at.isoformat() + "Z",
                        "ap_uptime": duplicate_threat.ap_uptime
                    }
                }, room=DASHBOARD_ROOM)
                return

            wps_val = payload.get("wps_info") or payload.get("wps") or "Unknown"
            wps_status = _get_wps_status_helper(wps_val)
            enc = payload.get("encryption") or payload.get("auth")
            dynamic_severity = calculate_network_severity(ssid, enc, wps_status)
            
            payload_severity = payload.get("severity")
            if payload_severity:
                final_severity = _normalize_severity(payload_severity, classification)
            else:
            ...
            _emit_socket_event(socketio, "threat_event", {"id": new_threat.id, "type": THREAT_DETECTED_EVENT, "timestamp": event_payload["timestamp"], "severity": new_threat.severity, "data": {"id": new_threat.id, "threat_type": new_threat.threat_type, "severity": new_threat.severity, "source_mac": new_threat.source_mac, "ssid": new_threat.ssid, "detected_by": new_threat.detected_by, "description": new_threat.description, "signal": event_payload["signal"], "channel": event_payload["channel"], "vendor": event_payload["manufacturer"], "is_resolved": new_threat.is_resolved, "created_at": new_threat.created_at.isoformat() + "Z", "ap_uptime": new_threat.ap_uptime}}, room=DASHBOARD_ROOM)
        except Exception as exc:
            db.session.rollback()
            LOGGER.exception("[THREAT] handle_new_threat failed for bssid=%s ssid=%s: %s", source_mac, ssid, exc)
        finally:
            db.session.remove()

    @socketio.on("deauth_attack")
    def handle_deauth_attack(payload):
        payload = sanitize_json_payload(payload or {})
        payload.pop("created_at", None)
        payload.pop("updated_at", None)

        target_bssid = _normalize_bssid(payload.get("target_bssid"))
        ssid = _normalize_ssid(payload.get("ssid") or payload.get("network_name"))
        sensor_id = _strict_sensor_id(payload.get("sensor_id"))
        description = sanitize_input(str(payload.get("description", "")), max_length=1024) or (
            f"External deauthentication attack on trusted network {ssid} ({target_bssid})"
        )
        timestamp = _utc_iso()

        if not target_bssid:
            LOGGER.warning("[DEAUTH_ATTACK] Received event with missing target_bssid — dropped")
            return

        try:
            duplicate = _find_recent_duplicate_threat("deauth_attack", target_bssid)
                ...
                _emit_socket_event(
                    ...
                    "threat_event",
                    {
                        ...
                        "data": {
                            ...
                            "severity": duplicate.severity,
                            "source_mac": duplicate.source_mac,
                            "ssid": duplicate.ssid,
                            "detected_by": duplicate.detected_by,
                            "description": duplicate.description,
                            "signal_strength": payload.get("attacker_rssi"),
                            "packet_count": payload.get("frame_count", 0),
                            "is_resolved": duplicate.is_resolved,
            ...
            LOGGER.info(
                "[DEAUTH_ATTACK] New threat id=%d bssid=%s ssid=%s sensor=%s",
                new_threat.id, target_bssid, ssid, sensor_id,
            )
            ...
            _emit_socket_event(
                ...
                "threat_event",
                {
                    ...
                    "data": {
                        ...
                        "created_at": new_threat.created_at.isoformat() + "Z",
                    },
                },
                room=DASHBOARD_ROOM,
            )
            ...
            db.session.rollback()
            LOGGER.exception("[DEAUTH_ATTACK] handle_deauth_attack failed bssid=%s: %s", target_bssid, exc)
        finally:
            db.session.remove()
    ...
    def handle_attack_command(payload):
        ...
        with _connected_clients_lock:
            client_info = connected_clients.get(request.sid) or {}
            is_registered_sensor = "sensor_id" in client_info
            
        if is_registered_sensor:
            LOGGER.critical(
                "[SECURITY] attack_command rejected: A registered sensor (sid=%s) attempted to issue an attack command! Possible compromise.",
                request.sid,
            )
            _emit_context_event(
                "attack_command_ack",
                {"status": "error", "message": "Forbidden: Sensors are strictly prohibited from issuing attack commands", "timestamp": _utc_iso()},
            )
            return

        try:
            requested_sensor_id = _strict_sensor_id(payload.get("sensor_id"))
            ...
            if network_snapshot is None:
                db_net_fallback = WiFiNetwork.query.filter_by(bssid=bssid).order_by(
                    WiFiNetwork.last_seen.desc()
                ).first()
                if db_net_fallback is None:
                    raise ValueError(f"Network {bssid} is unknown — never observed by any sensor")
                channel = db_net_fallback.channel or None
            else:
                channel = _safe_int(network_snapshot.get("channel"), default=0) or None

            best_sensor_id = get_best_sensor_for_bssid(bssid)
            sensor_id = (
                best_sensor_id
                if best_sensor_id and is_sensor_connected(best_sensor_id)
                else requested_sensor_id
            ...
            if sensor_id is None:
                with _connected_clients_lock:
                    for _fsid, _finfo in connected_clients.items():
                        if _finfo.get("client_type") == "sensor":
                            _candidate = _safe_int(_finfo.get("sensor_id"), default=0)
                            if _candidate and is_sensor_connected(_candidate):
                                sensor_id = _candidate
                                break
            if sensor_id is None:
                raise ValueError("attack_command missing sensor_id — no sensor is connected")
            ...
            _emit_socket_event(socketio, ATTACK_COMMAND_EVENT, {**command_payload, "timestamp": datetime.utcnow().isoformat() + "Z", "status": "dispatched", "routed_via_consensus": routed_via_consensus}, room=DASHBOARD_ROOM)
            _emit_socket_event(socketio, EXECUTE_ATTACK_EVENT, command_payload, room=sensor_sid)
            _emit_context_event("attack_command_ack", {"status": "ok", "sensor_id": sensor_id, "bssid": bssid, "channel": channel, "routed_via_consensus": routed_via_consensus, "timestamp": datetime.utcnow().isoformat() + "Z"})
        except Exception as exc:
