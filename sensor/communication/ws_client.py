...
class WSClient:
    def __init__(self, backend_url=None, token=None, sensor_id=None):
        ...
        # Removed duplicate ContainmentEngine initialization.
        # ThreatManager already instantiates the single global auto-containment engine.
    ...
    def _threat_listener(self):
        """توجيه أحداث التهديدات، اللوجات، وتغييرات الحالة للباك إند"""
        while self.is_running:
            try:
            ...
            if event_type:
                # 1. إرسال تغييرات حالة المحرك (عشان الكروت والجدول تتحدث)
                ...
                # 2. إرسال لوجات الهجوم (عشان التيرمينال يبقا لايف)
                elif event_type == "ATTACK_LOG":
                    bssid_raw = threat.get("bssid")
                    log_payload = {
                        "sensor_id": self._sensor_id_value(),
                        # الضربة القاضية: تحويل الـ MAC لحروف كبيرة إجبارياً
            ...
            # 3. معالجة التهديدات المكتشفة وتحديد الـ Severity
            event = threat.get("event")
            if not event or not isinstance(event, dict) or not event.get("bssid"):
                continue

            # المنطق الجديد: شبكة OPEN + إشارة قوية (>= -30) = HIGH Severity
            ...
            payload = {
                ...
                "action_status": "IDLE" if classification == "LEGIT" else threat.get("action_status", "IDLE"),
    ...
    def _should_process_scan(self, scan):
        ...
        current_ssid = scan.get("ssid") # الجديد
        was_hidden = scan.get("was_hidden", False) # الجديد
        ...
        # === الجديد: كسر الفلتر لو الشبكة اتكشفت أو اسمها اتغير ===
    ...
    def _update_last_sent_cache(self, payload):
        ...
        with self._scan_cache_lock:
            self.last_sent_cache[bssid] = {
                ...
                "ssid": payload.get("ssid"), # الجديد
                "was_hidden": payload.get("was_hidden", False), # الجديد
                "last_sent": time.time(),
            }
    ...
    def _build_scan_payload(self, scan):
        ...
        # Ensure we grab encryption and auth correctly from utils
        auth_val = scan.get("auth", "Unknown")
        enc_val = "OPEN" if auth_val == "OPEN" else "SECURED"
        ...
        return {
            ...
            "was_hidden": scan.get("was_hidden", False), # السطر ده هو الكوبري للباك إند
        } 
    ...
    def _execute_attack_command(self, target_bssid, channel):
        LOGGER.info(f"🚀 Manual attack triggered for: {target_bssid} on channel {channel}")
        from core.event_bus import event_queue
        # إرسال إشعار لمحرك التهديدات لبدء الاحتواء يدوياً
        self._queue_attack_ack("executing", target_bssid, "Manual attack initiated.")
        # ملاحظة: يمكنك استدعاء containment_engine مباشرة إذا كان متاحاً في السياق
    ...
    def _queue_attack_ack(self, status, target_bssid, message=None, ssid=None, channel=None, signal=None):
        ...
        # Send a single, enriched attack_ack event
