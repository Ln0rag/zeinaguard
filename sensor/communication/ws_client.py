...
class WSClient:
    def __init__(self, backend_url=None, token=None, sensor_id=None):
        ...
        # --- التعديل هنا: إنشاء نسخة موحدة من محرك الهجوم وربطها بطابور الإرسال ---
        ...
        self.containment_engine = ContainmentEngine(
            ...
            ack_callback=self._queue_attack_ack
        )
        # ------------------------------------------------------------------
    ...
    def _execute_attack_command(self, target_bssid, channel):
        try:
            ...
            # --- التعديل هنا: بننده على النسخة الموحدة بدل ما نعمل واحدة جديدة ---
            self.containment_engine.contain(target_bssid, clients, channel)
            
            LOGGER.info("[ATTACK PROCESS ENDED] target=%s channel=%s", target_bssid, channel)
        except Exception as exc:
