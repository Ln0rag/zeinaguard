...
class RiskEngine:
    ...
    def analyze(self, event):
        ...
        # تحويل بيانات الـ clients لرقم صحيح بدقة
        client_count = 0
        ...
        # --- 🛡️ منطق الشبكات المفتوحة العادي ---
        ...
        # --- 🚨 المنطق المعدل: الإعدام الفوري الذكي (Auto-Kill Logic) ---
        # لن يتم الهجوم إلا إذا كانت الشبكة مفتوحة، قريبة، وبها أجهزة متصلة فعلياً
        if is_open_network and smoothed_signal is not None and smoothed_signal > -60:
            if client_count > 0:
                score += 50  # رفعة قوية للسكور لتفعيل الـ ROGUE
                reasons.append(f"CRITICAL ALERT: Unauthorized Open Network with {client_count} Active Clients! (Auto-Kill Triggered)")
            else:
                score += 15 # سكور إضافي للمراقبة فقط بدون هجوم
        ...
        if is_trusted:
            ...
            # عتبة الـ 50 هي اللي بتبدأ عندها حالة الـ ATTACKING في الـ ThreatManager
            if score >= 50:
