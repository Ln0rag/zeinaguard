...
DEAUTH_COUNT = int(os.getenv("DEAUTH_COUNT", "20"))
...
def get_trusted_networks_dict():
    """Return dict of {bssid: ssid} from trusted file."""
    with open(TRUSTED_FILE) as f:
        data = json.load(f)
        if isinstance(data, list):
            return {b: None for b in data}
        if isinstance(data, dict):
            return data
        return {}

# --- SOC Auto-Containment Configuration ---
class AutoContainmentConfig:
    AUTO_CONTAIN_SCORE_THRESHOLD = 80 
    ATTACK_CYCLE_DURATION_SEC = 180
    EVALUATION_PAUSE_SEC = 10
    MAX_RE_ARM_CYCLES = 3
