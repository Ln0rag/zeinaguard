...
def main():
    ...
    def aggressive_watchdog():
        """خيط منفصل يراقب الهاردوير مباشرة ويفرض إعادة التشغيل من نظام التشغيل"""
        was_removed = False
        while True:
            import time, subprocess, os, sys
            time.sleep(3)
            res = subprocess.run(["iwconfig", selected_interface], capture_output=True, text=True)
            
            if "No such device" in res.stderr or selected_interface not in res.stdout:
                if not was_removed:
                    LOGGER.critical("!!! PHYSICAL ADAPTER [%s] REMOVED !!!", selected_interface)
                    update_status(sensor_status="error", message=f"Adapter {selected_interface} physically removed!")
                    was_removed = True
            else:
                if was_removed:
                    LOGGER.info("Adapter [%s] reconnected! Forcing full system restart...", selected_interface)
                    update_status(sensor_status="starting", message="Adapter reconnected. Rebooting sensor...")
                    time.sleep(4) # إعطاء لينكس وقت لتعريف الكارت
                    
                    subprocess.run(["ip", "link", "set", selected_interface, "down"])
                    subprocess.run(["iw", selected_interface, "set", "type", "monitor"])
                    subprocess.run(["ip", "link", "set", selected_interface, "up"])
                    
                    # إعادة تشغيل السكريبت بالكامل من الجذور
                    os.execv(sys.executable, [sys.executable] + sys.argv)

    # تشغيل المراقب العنيف في الخلفية
    threading.Thread(target=aggressive_watchdog, daemon=True, name="AggressiveWatchdog").start()

    update_status(sensor_status="monitoring", message=f"Wireless monitoring active on {selected_interface}")
    start_monitoring()
