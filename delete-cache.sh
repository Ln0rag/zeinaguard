...
clean_python_cache() {
    ...
    # Clean Replit-specific Python libraries
    if [ -d "$ROOT_DIR/.pythonlibs" ]; then
        size_before=$(get_dir_size "$ROOT_DIR/.pythonlibs")
        log "Removing Replit Python libraries (.pythonlibs) ($(format_size $size_before))..."
        rm -rf "$ROOT_DIR/.pythonlibs" 2>/dev/null || {
            if command -v sudo >/dev/null 2>&1; then
                sudo rm -rf "$ROOT_DIR/.pythonlibs" 2>/dev/null || true
            fi
        }
        TOTAL_SIZE_BEFORE=$((TOTAL_SIZE_BEFORE + size_before))
    fi
...
clean_build_artifacts() {
    ...
    # Clean Replit/Linux environment caches (.cache, .local, .upm)
    if [ -d "$ROOT_DIR/.cache" ]; then
        size_before=$(get_dir_size "$ROOT_DIR/.cache")
        log "Removing environment cache (.cache) ($(format_size $size_before))..."
        rm -rf "$ROOT_DIR/.cache" 2>/dev/null || {
            if command -v sudo >/dev/null 2>&1; then
                sudo rm -rf "$ROOT_DIR/.cache" 2>/dev/null || true
            fi
        }
        TOTAL_SIZE_BEFORE=$((TOTAL_SIZE_BEFORE + size_before))
    fi
    ...
    if [ -d "$ROOT_DIR/.local" ]; then
        size_before=$(get_dir_size "$ROOT_DIR/.local")
        log "Removing environment cache (.local) ($(format_size $size_before))..."
        rm -rf "$ROOT_DIR/.local" 2>/dev/null || {
            if command -v sudo >/dev/null 2>&1; then
                sudo rm -rf "$ROOT_DIR/.local" 2>/dev/null || true
            fi
        }
        TOTAL_SIZE_BEFORE=$((TOTAL_SIZE_BEFORE + size_before))
    fi

    if [ -d "$ROOT_DIR/.upm" ]; then
        size_before=$(get_dir_size "$ROOT_DIR/.upm")
        log "Removing Replit UPM cache (.upm) ($(format_size $size_before))..."
        rm -rf "$ROOT_DIR/.upm" 2>/dev/null || {
            if command -v sudo >/dev/null 2>&1; then
                sudo rm -rf "$ROOT_DIR/.upm" 2>/dev/null || true
            fi
        }
        TOTAL_SIZE_BEFORE=$((TOTAL_SIZE_BEFORE + size_before))
    fi
    ...
    # Remove previous zip exports
    if [ -f "$ROOT_DIR/zeinaguard.zip" ]; then
        size_before=$(get_dir_size "$ROOT_DIR/zeinaguard.zip")
        log "Removing previous export (zeinaguard.zip) ($(format_size $size_before))..."
        rm -f "$ROOT_DIR/zeinaguard.zip" 2>/dev/null || true
        TOTAL_SIZE_BEFORE=$((TOTAL_SIZE_BEFORE + size_before))
    fi
