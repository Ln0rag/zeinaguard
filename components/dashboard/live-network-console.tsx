...
import { Activity, ArrowUpDown, Clock, Search, Server, Shield, ShieldOff, Wifi, Users, Lock, Unlock, Loader2, CheckCircle2, Bell, BellOff } from 'lucide-react';
...
interface NotificationEvent {
  id: string;
  type: 'rogue' | 'attack_start' | 'attack_end' | 'attack_aborted' | 'purge' | 'network_trusted' | 'network_untrusted';
  title: string;
  time: string;
  bssid: string;
  ssid?: string;
...
function RouterUptimeValue({ baseSeconds, lastSeen }: { baseSeconds: number; lastSeen?: string | null }) {
  ...
  useEffect(() => {
    setDisplaySeconds((current) => {
      ...
      if (next < current - 120) return next;
...
const ATTACK_WATCHDOG_TIMEOUT_MS = 20000; // 20 seconds (5s attack + 3s evaluation + network overhead)

export function LiveNetworkConsole() {
  const [activeAttackBssids, setActiveAttackBssids] = useState<Set<string>>(new Set());
  const [attackQueue, setAttackQueue] = useState<LiveNetworkEvent[]>([]);
  const [startingAttackBssid, setStartingAttackBssid] = useState<string | null>(null);
  ...
  const { notifications, hasUnread: hasNewNotifications, markRead, clear: clearNotifications, addNotification } = useAlertStore();
  ...
  const pendingAttackToastsRef = useRef<Map<string, string | number>>(new Map());
      ...
      incoming.forEach(rawNet => {
        ...
        if (existing) {
          ...
          const nextNetwork: LiveNetworkEvent = {
            ...
            is_active: Boolean(norm.is_active) || Boolean(existing.is_active),
          };
  ...
  const { sendAttackCommand, isConnected } = useSocket({
    ...
    onAttackCommandAck: (event) => {
      const bssid = (event.bssid || '').toUpperCase();
      setStartingAttackBssid(prev => prev === bssid ? null : prev);
      
      const pendingId = pendingAttackToastsRef.current.get(bssid);
      if (event.status === 'ok') {
        setActiveAttackBssids(prev => new Set(prev).add(bssid));
        toast.success('Attack Started', { id: pendingId, description: `Deauth command relayed to sensor ${event.sensor_id}` });
      } else {
        toast.error('Attack Failed', { id: pendingId, description: event.message || 'Sensor rejected the command' });
      }
      if (bssid) pendingAttackToastsRef.current.delete(bssid);
    },
    onAttackAck: (event) => {
      const status = (event.status || '').toLowerCase();
      const attackData = event.attack || event;
      const bssid = (event.bssid || '').toUpperCase();
      ...
      try {
        if (['executed', 'finished', 'success', 'ok'].includes(status)) {
          toast.success('Attack Completed', { description: `Target ${(attackData as any).ssid || 'Unknown'} contained.` });
        } 
      } catch (err) {
        console.error('[WS] Notification layer failed during attack_ack:', err);
      } finally {
        setActiveAttackBssids(prev => {
          const next = new Set(prev);
          next.delete(bssid);
          if (event.bssid) next.delete(event.bssid); 
          return next;
        });
      }
    },
  ...
  useEffect(() => {
    processingRef.current = startingAttackBssid;
  }, [startingAttackBssid]);

  useEffect(() => {
    if (attackQueue.length > 0 && activeAttackBssids.size === 0 && !startingAttackBssid) {
      const nextNetwork = attackQueue[0];
      const bssidUpper = nextNetwork.bssid; // Always uppercase due to normalized insertion
      
      if (processingRef.current === bssidUpper) return;
      processingRef.current = bssidUpper;
      
      setStartingAttackBssid(bssidUpper);
      setAttackQueue(prev => {
        if (prev.length > 0 && prev[0].bssid === bssidUpper) return prev.slice(1);
        return prev;
      });
      
      try {
        const toastId = toast.loading('Dispatching attack…', { description: `Targeting ${nextNetwork.ssid || nextNetwork.bssid}` });
        pendingAttackToastsRef.current.set(bssidUpper, toastId);
        sendAttackCommand({ sensor_id: nextNetwork.sensor_id || 0, bssid: bssidUpper, channel: nextNetwork.channel || 0 });
      } catch (error) {
        toast.error('Attack Failed', { description: error instanceof Error ? error.message : 'Socket not connected' });
        setStartingAttackBssid(null);
      }
    }
  }, [attackQueue, activeAttackBssids.size, startingAttackBssid, sendAttackCommand]);

  useEffect(() => {
    if (!isConnected) {
      setActiveAttackBssids(new Set());
      setStartingAttackBssid(null);
    }
  }, [isConnected]);

  // Watchdog: automatically recover stuck attacks if the sensor silently fails to ACK
  useEffect(() => {
    if (!startingAttackBssid && activeAttackBssids.size === 0) return;
    
    const timer = setTimeout(() => {
      if (startingAttackBssid) {
        const pendingId = pendingAttackToastsRef.current.get(startingAttackBssid);
        toast.error('Attack Timeout', { 
          id: pendingId, 
          description: 'Sensor failed to acknowledge the command.' 
        });
        pendingAttackToastsRef.current.delete(startingAttackBssid);
        setStartingAttackBssid(null);
      }
      
      if (activeAttackBssids.size > 0) {
        activeAttackBssids.forEach(bssid => {
          const pendingId = pendingAttackToastsRef.current.get(bssid);
          toast.error('Attack Stalled', { 
            id: pendingId, 
            description: 'Execution timed out without completion.' 
          });
          pendingAttackToastsRef.current.delete(bssid);
        });
        setActiveAttackBssids(new Set());
      }
    }, ATTACK_WATCHDOG_TIMEOUT_MS);
    
    return () => clearTimeout(timer);
  }, [startingAttackBssid, activeAttackBssids]);
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      if (isOfflineRef.current) return;
      try {
        const res = await fetch(`${apiBase}/api/wifi-networks/history`, { cache: 'no-store' });
        if (!res.ok) {
          // Graceful fallback: try the legacy active endpoint
          const fallback = await fetch(`${apiBase}/networks/active`, { cache: 'no-store' });
  ...
  const handleAttack = (network: LiveNetworkEvent) => {
    const bssidUpper = network.bssid.toUpperCase();
    
    const isRunning = activeAttackBssids.has(bssidUpper);
    const isStarting = startingAttackBssid === bssidUpper;
    const isQueued = attackQueue.some(n => n.bssid === bssidUpper);
    
    if (isRunning || isStarting || isQueued) {
      toast.error('Duplicate Attack Rejected', { description: `${network.ssid || network.bssid} is already queued or running.` });
      return;
    }

    setAttackQueue(prev => {
      if (prev.some(n => n.bssid === bssidUpper)) return prev;
      return [...prev, { ...network, bssid: bssidUpper }]; // Normalize ON INSERTION
    });
    toast.success('Attack Queued', { description: `Targeting ${network.ssid || network.bssid}` });
  };
  ...
  const handleTrust = async (network: LiveNetworkEvent) => {
     ...
     try {
       ...
       addNotification({
         type: 'network_trusted',
         title: 'NETWORK TRUSTED',
         bssid: bssid,
         ssid: network.ssid || 'Hidden',
         channel: network.channel,
         signal: network.signal,
         manufacturer: network.manufacturer || 'Unknown Mfr',
       });
     } catch (error) {
  ...
  const handleUntrust = async (network: LiveNetworkEvent) => {
     const bssid = network.bssid;
     setTrustingBssids(prev => new Set(prev).add(bssid));
     try {
       const response = await fetch(`${apiBase}/untrust`, {
         method: 'POST',
         headers: { 'Content-Type': 'application/json' },
         body: JSON.stringify({ bssid }),
       });
       if (!response.ok) throw new Error('Failed to untrust network');
       toast.info('Network removed from trusted list');
       
       addNotification({
         type: 'network_untrusted',
         title: 'TRUST REVOKED',
         bssid: bssid,
         ssid: network.ssid || 'Hidden',
         channel: network.channel,
       ...
       toast.error('Untrust Failed', { description: error instanceof Error ? error.message : 'Network issue' });
     } finally {
       setTrustingBssids(prev => {
         const next = new Set(prev);
         next.delete(bssid);
  ...
  const getNotificationColor = (type: string) => {
    switch (type) {
      ...
      case 'network_trusted': return 'border-emerald-500 text-emerald-400';
      case 'network_untrusted': return 'border-orange-500 text-orange-400';
    ...
    <div className="space-y-6">
      <Card className="bg-slate-900 overflow-hidden border-none shadow-none py-0">
                    ...
                    <ScrollArea className="h-[600px]">
                      {notifications.length === 0 ? (
                        ...
                        notifications.map((notif) => {
                          ...
                          const isHardware = !isPurge && notif.type !== 'network_trusted' && Boolean(notif.sensor_id);
                            ...
                            >
                              <div className="flex flex-col gap-2.5">
                                <div className="flex justify-between items-start">
                                  <h4 className="font-bold text-[13px] tracking-wide uppercase flex items-center gap-1.5">
                                    {notif.type === 'network_trusted' && <CheckCircle2 className="w-4 h-4 text-emerald-400" />}
                                    {notif.type === 'network_untrusted' && <ShieldOff className="w-4 h-4 text-orange-400" />}
                                    {notif.title}
                                  </h4>
          ...
          <CardContent className="px-0">
            {loading ? (
              <div className="flex h-64 items-center justify-center text-emerald-500/50"><Activity className="mr-2 h-5 w-5 animate-spin" />Initializing...</div>
            ) : (
                          ...
                          <td className="px-3 py-4">
                            <div className="flex items-center justify-center gap-2">
                                <Button
                                  size="sm"
                                  variant="destructive"
                                  disabled={activeAttackBssids.has(network.bssid.toUpperCase()) || startingAttackBssid === network.bssid.toUpperCase() || attackQueue.some(n => n.bssid === network.bssid.toUpperCase())} 
                                  onClick={() => handleAttack(network)}
                                  className="relative h-8 px-3"
                                >
                                  {activeAttackBssids.has(network.bssid.toUpperCase()) ? (
                                    <>
                                      <Loader2 className="h-3 w-3 animate-spin mr-2" />
                                      Attacking
                                    </>
                                  ) : startingAttackBssid === network.bssid.toUpperCase() ? (
                                    <>
                                      <Loader2 className="h-3 w-3 animate-spin mr-2" />
                                      Starting
                                    </>
                                  ) : attackQueue.some(n => n.bssid === network.bssid.toUpperCase()) ? (
                                    <>
                                      <Loader2 className="h-3 w-3 animate-spin mr-2" />
                                      Queued
                                    </>
                                ...
                                {isTrusted && (
                                  <button
                                    onClick={() => handleUntrust(network)}
                                    disabled={isTrusting}
                                    className="group relative flex items-center justify-center px-2 py-1 rounded-md border transition-all duration-200 overflow-hidden w-[100px] h-8
                                  ...
                                  >
                                    {isTrusting ? (
                                      ...
                                      <>
                                        <div className="flex items-center gap-1.5 transition-opacity duration-200 group-hover:opacity-0 group-hover:absolute">
                                          <CheckCircle2 className="h-3 w-3" />
                                          <span className="text-[10px] font-bold uppercase tracking-wider">Trusted</span>
                                        </div>
                                        <div className="flex items-center gap-1.5 absolute opacity-0 transition-opacity duration-200 group-hover:opacity-100 group-hover:relative">
                                          <ShieldOff className="h-3 w-3" />
                                          <span className="text-[10px] font-bold uppercase tracking-wider">Untrust</span>
                                        </div>
                                      </>
                                    )}
                                  </button>
